...
def stringify(a, da, digits=2):
    ...
    """
    template = '({{:.{0:d}f}} \\pm {{:.{0:d}f}})'.format(digits)
...
def fractions(acro, name, pKa, pH1, pH2, create_table=True):
    ...
    ax.plot(pH, acid, c='r', lw=2.5, label='[AH][%]')
    ax.plot(pH, anion, c='g', lw=2.5, label='[A$^{-}$][%]', ls='--')
    ax.legend()

    ax.axvline(pH1, c='b', ls=':', lw=3)
    ax.axvline(pH2, c='b', ls=':', lw=3)
...
def draw_plot(T, k, dk, result, pH, draw_png=False):
    ...
    Args:
        ...
        draw_png (bool, optional): determines whether to create png image in addition to pdf 
    ...
    if draw_png:
        fig.savefig(acro + '.png', dpi=250)

def draw_double_plot(T1, k1, dk1, result1, pH1, T2, k2, dk2, result2, pH2, kacid, kanion, draw_png=False):
    """Summary
    ...
    ax.errorbar(x1, y1, yerr=yerr1, fmt='^', c='r')
    ax.errorbar(x2, y2, yerr=yerr2, fmt='o', c='b')
    xs = np.linspace(*xlims)
    ys1 = np.log10(result1['a']) - result1['ea'] / 8.314 * xs * 1000 / 2.3026
    ys2 = np.log10(result2['a']) - result2['ea'] / 8.314 * xs * 1000 / 2.3026
    ax.plot(xs, ys1, lw=2, c='r', label='pH = ' + str(pH1))
    ax.plot(xs, ys2, lw=2, ls='--', c='b', label='pH = ' + str(pH2))

    # plotting 'pure' data
    ax.scatter(1. / temps, np.log10(kacid),  c='k', marker='x', s=70, label='k_acid', zorder=99)
    ax.scatter(1. / temps, np.log10(kanion), c='k', marker='+', s=70, label='k_anion', zorder=99)
    ...
    # legend
    line1 = mlines.Line2D([], [], color='red' , marker='^', ls='-',  markersize=8, label='pH = ' + str(pH1))
    line2 = mlines.Line2D([], [], color='blue', marker='o', ls='--', markersize=8, label='pH = ' + str(pH2))
    line3 = mlines.Line2D([], [], color='black', marker='x', ls=':', markersize=8, label='k$_{acid}$')
    line4 = mlines.Line2D([], [], color='black', marker='+', ls=':', markersize=8, label='k$_{anion}$')
    ...
    if draw_png:
        fig.savefig(acro + '.double.png', dpi=250)
...
if __name__ == '__main__':
    ...
    for dataset in read_data_file(fn):
        ...
        draw_double_plot(T1, k1, dk1, result1, pH1, T2, k2, dk2, result2, pH2, kacid, kanion, draw_png=0)
